...
import { z } from "zod";
import { TOPICS, COMPILED_AT, Topic } from "./data.js";
...
function findTopic(idOrName: string): Topic | undefined {
  const key = idOrName.trim().toLowerCase();
  const direct = BY_KEY.get(key);
  if (direct) return direct;
  for (const t of TOPICS) if (t.platform.toLowerCase() === key) return t;
  const partial = TOPICS.filter(
    (t) => t.id.includes(key) || t.platform.toLowerCase().includes(key)
  );
  if (partial.length === 1) return partial[0];
  return undefined;
}

function summarize(t: Topic) {
  return {
    id: t.id,
    platform: t.platform,
    category: t.category,
...
function search(query: string): Topic[] {
  const terms = query.toLowerCase().split(/\s+/).filter(Boolean);
  if (terms.length === 0) return [];
  const scored: Array<{ t: Topic; s: number }> = [];
  for (const t of TOPICS) {
...
const server = new McpServer(
  { name: "solana-research-library", version: "1.0.0" },
  { instructions:
      "Query a curated Solana/web3 earning-rail intelligence DB: 25 platforms probed 2026-09-04 across x402 payment rails, bounties, agent-bounty venues, quests, write-to-earn, testnets, data platforms and affiliates. " +
...
server.tool(
  "list_topics",
  "List all 25 entries in the earning-rail intelligence DB (compiled 2026-09-04): platform, category, verdict, payout rail, KYC, chains.",
  {},
  async () => ({
    content: [{
      type: "text",
...
server.tool(
  "get_topic",
  "Get one entry by id, exact platform name, or unique fragment — returns the FULL evidence notes and the source brief it came from.",
  { idOrName: z.string().describe("Entry id, platform name, or unique fragment, e.g. 'x402', 'grass', 'apify-store'") },
  async ({ idOrName }) => {
    const t = findTopic(idOrName);
    if (!t) {
      const matches = TOPICS.filter((x) =>
        (x.platform + " " + x.id).toLowerCase().includes(idOrName.trim().toLowerCase())
      ).map((x) => x.platform);
...
server.tool(
  "search_topics",
  "Keyword search across platform names, categories, payout rails, KYC gates and full notes. All terms must match; ranked by term frequency (name hits weigh more).",
  { query: z.string().describe("Keywords, e.g. 'USDC Solana', 'no KYC', 'dead fiat'") },
  async ({ query }) => {
...
server.tool(
  "stats",
  "Dataset shape: entry count, compile date, verdict mix, category mix, KYC-free share, crypto-rail share, live-rail shortlist.",
  {},
  async () => {
